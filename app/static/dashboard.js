...
async function loadDashboard() {
  ...
  const watching = [...(portfolio.watching_decisions || [])].sort((a,b)=>(actionPriority[a.decision?.action] ?? 9)-(actionPriority[b.decision?.action] ?? 9)).slice(0,8);
  $("#watchActions").innerHTML = watching.length ? watching.map(row => actionCard(row.symbol,row.name,row.decision?.action,(row.decision?.reasons || []).slice(0,2).join("；"))).join("") : empty("尚未加入觀察股票。");
