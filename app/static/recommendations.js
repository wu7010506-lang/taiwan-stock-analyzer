  ...
  try {
    ...
    grid.innerHTML = rows.map((row, index) => `
      <article class="recommendation-card" data-symbol="${row.symbol}" tabindex="0" role="link">
        ...
        <div class="recommendation-head"><div><span>${row.evidence_decision || row.rating}</span><h2>${row.symbol} ${row.name}</h2><small>${row.market} · ${row.industry || "產業未分類"}${row.profile === "evidence_based" ? ` · ${row.industry_model}` : ""}</small></div><strong>${number(row.score)}<i>分</i></strong></div>
        <div class="score-bar"><i style="width:${Math.min(row.score, 100)}%"></i></div>
        <div class="factor-grid">${factorCards(row)}</div>
        <div class="institution-risk ${row.speculation_risk === "高" ? "high" : row.speculation_risk === "中" ? "medium" : "low"}"><strong>法人推動 ${row.institution_influence}</strong><span>炒作風險 ${row.speculation_risk}</span><small>${row.institution_influence_reasons.length ? row.institution_influence_reasons.join("｜") : "未見短線急漲與法人集中買超同時出現"}</small></div>
        <ul class="reason-list">${[...(row.industry_adjustments || []), ...row.reasons].slice(0, 5).map(reason => `<li>${reason}</li>`).join("")}</ul>
        ${(row.invalidation_conditions?.length || row.risks.length) ? `<div class="risk-note">風險／失效條件：${[...(row.invalidation_conditions || []), ...row.risks].slice(0, 5).join("、")}</div>` : ""}
