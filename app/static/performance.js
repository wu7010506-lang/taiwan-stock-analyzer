(() => {
  ...
  function renderStrategy(data) {
    ...
    const oosSummary = oos && oos.periods
      ? `樣本外（${data.out_of_sample_start} 起）${oos.periods} 期：策略 ${signed(oos.total_return_percent)}、指數 ${signed(oos.benchmark_total_return_percent)}、超額 ${signed(oos.excess_return_percent)}、最大回撤 ${signed(oos.max_drawdown_percent)}。`
  ...
  function syncControls() {
    ...
    $("#profileControl").hidden = !["live", "factors"].includes(mode);
